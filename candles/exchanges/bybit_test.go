...
func TestBybitExchange_RetryLogic(t *testing.T) {
	exchange := NewBybitExchange()
...
func TestBybitExchange_SymbolConversion(t *testing.T) {
	exchange := NewBybitExchange()
	pairs := []string{"BTC-USDT", "ETH-USDT", "SOL-USDT"}
	exchange.convertPairsToBybitFormat(pairs)

	t.Run("ExchangeToCanonical", func(t *testing.T) {
		symbol, err := exchange.FromExchangeSymbol("BTCUSDT")
		assert.NoError(t, err)
		assert.Equal(t, "BTC-USDT", symbol)

		symbol, err = exchange.FromExchangeSymbol("ETHUSDT")
		assert.NoError(t, err)
		assert.Equal(t, "ETH-USDT", symbol)

		// Test non-existent symbol
		_, err = exchange.FromExchangeSymbol("INVALIDPAIR")
		assert.Error(t, err)
		assert.Contains(t, err.Error(), "not found in cache")
	})

	t.Run("ConvertPairsToBybitFormat", func(t *testing.T) {
		bybitPairs := exchange.convertPairsToBybitFormat([]string{"BTC-USDT", "ETH-USDT"})
		assert.Equal(t, []string{"BTCUSDT", "ETHUSDT"}, bybitPairs)
...
func TestBybitExchange_ConnectionHealthTracking(t *testing.T) {
	exchange := NewBybitExchange()
...
func TestBybitExchange_ConnectionManagement(t *testing.T) {
	exchange := NewBybitExchange()
...
func TestBybitExchange_Channels(t *testing.T) {
	exchange := NewBybitExchange()
	...
	// Note: Errors are now logged directly in the exchange connector
	// Future enhancement: Add centralized error handling, metrics, and alerting
}

func TestBybitExchange_TradeNormalization(t *testing.T) {
	exchange := NewBybitExchange()
	// Setup symbol mapping
	exchange.exchangeToCanonical["BTCUSDT"] = "BTC-USDT"
	...
	tradeData := struct {
		...
		Size:      "0.1",
		Side:      "Buy",
		TradeTime: 1640995200000, // 2022-01-01 00:00:00 UTC in milliseconds
		TradeID:   "123456",
	}
	...
	assert.Equal(t, models.ExchangeBybit, trade.Exchange)
...
func TestBybitExchange_InvalidData(t *testing.T) {
	exchange := NewBybitExchange()
	exchange.exchangeToCanonical["BTCUSDT"] = "BTC-USDT"
